...
int pinnacle_set_shutdown(const struct device *dev, bool enabled) {
    ...
    if (((sys_cfg & PINNACLE_SYS_CFG_SHUTDOWN) != 0) == enabled) {
        LOG_WRN("Shutdown already set to %s, doing nothing", (enabled ? "on" : "off"));
    }

    LOG_INF("Setting shutdown: %s", (enabled ? "on" : "off"));
    ...
    if (!enabled) {
        ...
        LOG_INF("Shutdown readback: %s", (sys_cfg & PINNACLE_SYS_CFG_SHUTDOWN) ? "on" : "off");
    }
